<?php
$slides = get_sub_field('slides');
$totalSlides = sizeof($slides);
?>
<section <?php AW_Helpers::SectionAttrs('caro-11'); ?>>
  <div class="wrap">
    <div class="slides-wrapper">
      <div class="slides">
        <?php
          while ( have_rows('slides') ) :
            the_row();
            $title = get_sub_field('title');
            $image = get_sub_field('image');
            $bgColor = get_sub_field('bg_color');
        ?>
          <div class="slide">
            <div class="img"><img src="<?php echo $image['url']; ?>" alt="<?php echo $image['alt']; ?>" class="img-fluid"></div>
            <?php if ( $title ) : ?>
              <h2 class="titlebar with-bg" style="color: <?php echo $bgColor; ?>">
                <span class="txt"><?php echo $title; ?></span>
                <span class="bg"></span>
              </h2>
            <?php endif; ?>
            <div class="txt-area">
              <?php if ( $title ) : ?>
                <h3 class="title"><span><?php echo $title; ?></span></h3>
              <?php endif; ?>

              <div class="msg format" style="background-color: <?php echo $bgColor; ?>;">
                <?php the_sub_field('description'); ?>
              </div>

              <?php if ( $totalSlides > 1 ) : ?>
                <div class="slides-nav">
                  <button type="button" class="btn-prev icon-caret-left-circle"></button>
                  <button type="button" class="btn-next icon-caret-right-circle"></button>
                </div>
              <?php endif; ?>
            </div>
          </div>
        <?php endwhile; ?>
      </div>
    </div>
    <!-- <div class="btm-slides-nav"></div> -->
  </div>
</section>
